...
export function ScratchpadPanel({ projectId }: { projectId: string }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const row = data as { scratch_content?: string | null } | null
      setDraft(typeof row?.scratch_content === 'string' ? row.scratch_content : '')
      ...
      {open ? (
        ...
        >
          <div className="flex items-center justify-between gap-2">
            <span className="text-sm font-semibold text-[var(--foreground)]">Scratch Pad</span>
            <button
              type="button"
